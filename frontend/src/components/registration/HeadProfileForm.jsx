...
export default function HeadProfileForm({ familyData = {}, onNext, loading, setLoading }) {
  const [submitError, setSubmitError] = useState('');
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (typeof onNext === 'function') {
        onNext({ ...data, familyId: result.familyId });
      }
    } catch (error) {
      console.error('Form submission error:', error);
    }
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-blue-700 mb-6">Head of Family Profile</h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Card>
          <CardHeader className="bg-blue-50 border-b border-blue-200 rounded-t-lg px-6 py-4">
            <CardTitle className="text-lg font-semibold text-blue-800">Head of Family Details</CardTitle>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  <option value="">Select City</option>
                  {selectedState &&
                    indianStates[selectedState]?.sort().map((city) => (
                      <option key={city} value={city}>{city}</option>
                    ))}
                </select>
                {errors.city && <p className="mt-1 text-sm text-red-600">{errors.city.message}</p>}
